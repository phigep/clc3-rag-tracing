...
The high-level goal of our project is a retrieval-augmented generation pipeline (prototypical) that runs on kubernetes, with built-in tracing of requests, retrieved contexts, and generated responses. It should allow the use of local LLMs (vLLM), which could help with scalability and security. A scalable and understandable (tracing!) RAG pipeline helps to refine the retrieval and generation part.
...
We build a custom image for the Python project instead of the hayhooks image. (See deployment for reasons). 

The project itself uses **python 3.12 and uv** as pmanager. If new packages are added to this prototype during further implementation, a requirements.txt needs to be exported from the pyproject.toml!  
...
- Ollama container running an Ollama server (https://ollama.com), which serves the used model.  
- Rest API container that runs a small exemplary RAG pipeline using Haystack (https://haystack.deepset.ai) with a single GET Call on a FASTAPI as entry point.
...
8. Execute streamlit run sample.py
...
- A Dashboard that shows an overview of
    ...
    - A Median Latency
    - and other metrics that can be added on the custom dashboards.
...
Secrets and Configmap was also a critical learning, as multiple potentially costly API Keys can and are used for this project. We initially started with an .env file but quickly realized that this wont be an option for the actual deployment.

### Outlook
There are a lot of areas with room for improvement, we only name a few here. It has to be understood that this is a simple prototype using the most basic approaches to cut down on coding effort a bit.
